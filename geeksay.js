...
function getGeeksayText(text) {
  const lowerCaseText = text.toLowerCase();
  return translations.hasOwnProperty(lowerCaseText)
    ? translations[lowerCaseText]
    : text;
}

function getGeeksayWord(text) {
  return isNumeric(text) ? (text >>> 0).toString(2) : getGeeksayText(text);
}

function geeksay(text) {
  const input = Array.isArray(text) ? text : String(text).split(" ");
  return input.map(getGeeksayWord).join(" ");
}
